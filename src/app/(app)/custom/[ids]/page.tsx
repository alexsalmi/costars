...
export default async function CustomGame({ params }: ICustomGameProps) {
  ...
  targetEntity.credits = (await getCredits(targetEntity.id, 'person')).map(credit => credit.id);
  starterEntity.credits = (await getCredits(starterEntity.id, 'person')).map(credit => credit.id);
