...
class LocalStorageService {
  ...
  getSolutions = (): Array<Solution> => {
    ...
    window.localStorage.setItem(
      KEY_SOLUTIONS,
      btoa(JSON.stringify(initialSolutions)),
    );
  ...
  getDailyStats = (): DailyStats => {
    ...
    window.localStorage.setItem(
      KEY_DAILY_STATS,
      btoa(JSON.stringify(initialDailyStats)),
    );
  ...
  getUnlimitedStats = (): UnlimitedStats => {
    ...
    window.localStorage.setItem(
      KEY_UNLIMITED_STATS,
      btoa(JSON.stringify(initialUnlimitedStats)),
    );
    window.localStorage.setItem(KEY_FRESH_STATUS, 'fresh');
  ...
  isFresh = () => {
    return (
      typeof window !== 'undefined' &&
      window.localStorage.getItem(KEY_FRESH_STATUS) !== null
    );
  };
}
