...
export const supabase_getSolution = async (uuid: string): Promise<Solution> => {
  ...
  const { data } = await supabase.from('Solutions').select().eq('id', uuid);

  if (!data || data.length === 0) throw Error('Invalid uuid');

  return data[0];
};

export const supabase_saveSolution = async (
...
export const supabase_getUserDailySolutions = async (
  user_id: string,
...
export const supabase_getDailySolutions = async (
  daily_id: number,
): Promise<Array<Solution>> => {
  ...
  if (!data || data.length === 0) throw Error('Invalid daily_id');
